...
type IUtility interface {
	...
	//对一条交易进行签名，并返回签名结果
	SignTransaction(priKey []byte, tx []byte, extra []byte) (signature []byte, err error)
	//将未签名的原始交易与签名进行绑定，返回一个签名后的交易
	BindTxAndSignature(tx []byte, signs [][]byte, extra []byte) (signedTx []byte, err error)
	//根据交易内容，计算交易Hash
	CalcTxHash(tx []byte, extra []byte) (hash []byte, err error)
	//将签名后的交易广播到网络中,如果发送交易需要手续费，指定最多支付的手续费
	SendTx(tx []byte, fee *AmountAsset, extra []byte) (txID []byte, err error)
	//根据交易ID获得交易的基本信息
